...
static bool seg_fault(vmm_segment_t *segment, uintptr_t address, int flags) {
    ...
    arch_vmm_map(segment->address_space, MATH_FLOOR(address, ARCH_PAGE_SIZE), PADDR(segment, MATH_FLOOR(address, ARCH_PAGE_SIZE)), segment->protection, segment->cache, mapflags);
    return true;
}
