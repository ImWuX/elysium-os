...
static bool seg_fault(vmm_segment_t *segment, uintptr_t address, int flags) {
    ...
    seg_fixed_data_t *data = (seg_fixed_data_t *) segment->driver_data;
    uintptr_t paddr = data->phys + ROUND_DOWN(address - (uintptr_t) data->base, ARCH_PAGE_SIZE);
    arch_vmm_map(segment->address_space, ROUND_DOWN(address, ARCH_PAGE_SIZE), paddr, segment->protection, ARCH_VMM_FLAG_NONE);
