...
void initialize_fs() {
    void *bpb_page = request_page();
    map_memory(bpb_page, bpb_page);

    if(!read(0, 1, bpb_page)) {
        printe("FS could not be initialized. BPB read failed.");
        return;
    }
    ...
    cluster_buffer = request_linear_pages(pages_per_cluster);
    for(uint32_t i = 0; i < pages_per_cluster; i++) {
        void *address = cluster_buffer + i * 0x1000;
        map_memory(address, address);
    }

    // TODO: the way we are creating "physical and then the offset thingy here is bad"
    void *fat_physical = (uint32_t *) request_linear_pages(bpb->sectors_per_fat / 8 + (bpb->sectors_per_fat % 8 > 0 ? 1 : 0));
    fat = (uint32_t *) ((uint64_t) fat_physical + 0xFFFF800000000000);
    if(!read(bpb->bios_param_block.reserved_sector_count, bpb->sectors_per_fat, (void *) fat_physical)) {
        printe("FS could not be initialized. FAT read failed.");
        return;
    }
}
