...
static void lapic_write(uint32_t offset, uint32_t data) {
    *(volatile uint32_t *) HHDM((msr_read(MSR_APIC_BASE) & BASE_MASK) + offset) = data;
}

static uint32_t lapic_read(uint32_t offset) {
    return *(volatile uint32_t *) HHDM((msr_read(MSR_APIC_BASE) & BASE_MASK) + offset);
}
...
void apic_eoi(uint8_t interrupt_vector) {
    if(lapic_read(REG_IN_SERVICE_BASE + interrupt_vector / 32 * 0x10) & (1 << (interrupt_vector % 32))) lapic_write(REG_EOI, 1);
}
