...
static x86_64_interrupt_priority_t g_ipl_to_interrupt_map[] = {
    [ARCH_INTERRUPT_IPL_NORMAL] = X86_64_INTERRUPT_PRIORITY_NORMAL - 1,
    [ARCH_INTERRUPT_IPL_IPC] = X86_64_INTERRUPT_PRIORITY_IPC - 1,
    [ARCH_INTERRUPT_IPL_CRITICAL] = X86_64_INTERRUPT_PRIORITY_CRITICAL - 1
};

static arch_interrupt_ipl_t g_interrupt_to_ipl_map[] = {
    [X86_64_INTERRUPT_PRIORITY_NORMAL - 1] = ARCH_INTERRUPT_IPL_NORMAL,
    [X86_64_INTERRUPT_PRIORITY_IPC - 1] = ARCH_INTERRUPT_IPL_IPC,
    [X86_64_INTERRUPT_PRIORITY_CRITICAL - 1] = ARCH_INTERRUPT_IPL_CRITICAL
};
...
static void ipl_set(x86_64_interrupt_priority_t priority) {
    asm volatile("mov %0, %%cr8" : : "r" ((uint64_t) priority));
}

static x86_64_interrupt_priority_t ipl_get() {
    uint64_t ipl;
    asm volatile("mov %%cr8, %0" : "=r" (ipl));
    return (x86_64_interrupt_priority_t) ipl;
}

void interrupt_handler(x86_64_interrupt_frame_t *frame) {
