...
time_t g_monotonic_time = {};
static spinlock_t g_lock = SPINLOCK_INIT;
static list_t g_timers = LIST_INIT; // TODO: this needs to be a vector instead of a list
...
void time_advance(time_t length) {
    ...
    LIST_FOREACH(&g_timers, elem) {
        ...
        if(timer->deadline.seconds > g_monotonic_time.seconds) continue;
        if(timer->deadline.seconds == g_monotonic_time.seconds && timer->deadline.nanoseconds > g_monotonic_time.nanoseconds) continue;
...
timer_t *timer_create(time_t length, void (* callback)(timer_t *timer)) {
    ...
    spinlock_acquire(&g_lock);
    timer->deadline = time_add(g_monotonic_time, length);
