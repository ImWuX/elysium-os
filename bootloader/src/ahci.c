...
static void configure_port(hba_port_t *port) {
    ...
    for(int i = 0; i < 32; i++) {
        ...
        void *command_table_address = pmm_request_page();
        vmm_map_memory(command_table_address, command_table_address);
...
void initialize_ahci_device(uint64_t bar5_address) {
    ...
    vmm_map_memory((void *) hba_mem, (void *) hba_mem);
